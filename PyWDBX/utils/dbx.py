...
def get_table_from_hash(tbl_hash):
    for x in TBL_HASHES:
        if TBL_HASHES[x] == tbl_hash:
            return x
    raise Exception("Unrecognized Hash "+str(tbl_hash))

def get_hash_from_table(tbl_name):
    for x in TBL_HASHES:
        if x.lower() == tbl_name.lower():
            return TBL_HASHES[x]
    raise Exception("Unknown table"+tbl_name)
